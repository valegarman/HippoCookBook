%% Notebook_for_checking_POTometry project
% PA and AMF, 2026

% Let's go one session by one, checking the results and reanylizing what is
% needed.

%% WT10_251027_sess1 
% Eliminating some neurons because of bad waveforms, reanalizing, and
% improving some analysis
processSession('basepath','X:\unindexedSubjects\wt10\wt10_251027_sess1','digital_optogenetic_channels',[],'analog_optogenetic_channels',[],'promt_hippo_layers',true);
...
%% WT10_251028_sess2
% 0. Gui session, check bad channels
processSession('basepath','X:\unindexedSubjects\wt10\wt10_251028_sess2','digital_optogenetic_channels',[],'analog_optogenetic_channels',[],'promt_hippo_layers',true);

session = sessionTemplate(pwd,'showGUI',true);
% 1. Hippocampal layers
...
eventSpikeThreshold = .5;

[ripples,SW] = rippleMasterDetector('rippleChannel',rippleChannel,'SWChannel',SWChannel,'force',true,'skipStimulationPeriods',true,'thresholds',rippleMasterDetector_threshold,'eventSpikeThreshold',eventSpikeThreshold); 
...
%% wt10_251029_sess3
processSession('basepath','X:\unindexedSubjects\wt10\wt10_251029_sess3','digital_optogenetic_channels',[],'analog_optogenetic_channels',[],'promt_hippo_layers',true);

session = loadSession;
TheStateEditor(session.general.name);

% Hippocampal layers
[hippocampalLayers] = getHippocampalLayers('force',true,'promt',true,'removeRipplesStimulation', true);
...
plotSummary('showTagCells',false,'use_deltaThetaEpochs',false);
%% wt10_251030_sess4
processSession('basepath','X:\unindexedSubjects\wt10\wt10_251030_sess4','digital_optogenetic_channels',[],'analog_optogenetic_channels',[],'promt_hippo_layers',true);

%% wt10_251031_sess5
processSession('basepath','X:\unindexedSubjects\wt10\wt10_251031_sess5','digital_optogenetic_channels',[],'analog_optogenetic_channels',[],'promt_hippo_layers',true);
...
% Brain Region and CellExplorer
session = assignBrainRegion('showPowerProfile','theta','showEvent','ripples','eventTwin',[-0.05 0.05]);

session = loadSession;
excludePulsesIntervals = true;
cell_metrics = ProcessCellMetrics('session', session,'forceReload',true,'excludeIntervals',excludePulsesIntervals,'excludeMetrics',{'deepSuperficial'}); % after CellExplorar
...
%% wt10_251103_sess6
processSession('basepath','X:\unindexedSubjects\wt10\wt10_251103_sess6','digital_optogenetic_channels',[],'analog_optogenetic_channels',[],'promt_hippo_layers',true);
...
session = loadSession;   
cell_metrics = ProcessCellMetrics('session', session,'forceReload',true,'excludeIntervals',[],'excludeMetrics',{'deepSuperficial'}); % after CellExplorar
...
% Exclude optogenetic intervals
targetFile = dir('*optogeneticPulses.events.mat'); load(targetFile.name);

[ripples,SW] = rippleMasterDetector('rippleChannel',rippleChannel,'SWChannel',SWChannel,'force',true,'skipStimulationPeriods',false,'thresholds',rippleMasterDetector_threshold,'eventSpikeThreshold',eventSpikeThreshold,'excludeIntervals',optoPulses.timestamps); 
psthRipples = spikesPsth([],'eventType','ripples','numRep',500,'force',true,'minNumberOfPulses',10);
getSpikesRank('events','ripples');
...
session = loadSession;
excludePulsesIntervals = optoPulses.timestamps;
cell_metrics = ProcessCellMetrics('session', session,'forceReload',true,'excludeIntervals',excludePulsesIntervals,'excludeMetrics',{'deepSuperficial'}); % after CellExplorar
...
%% wt10_251114_sess14
% Sleep Score
SleepScoreMaster(pwd,'noPrompts',true,'overwrite', true);
...
[ripples,SW] = rippleMasterDetector('rippleChannel',rippleChannel,'SWChannel',SWChannel,'force',true,'skipStimulationPeriods',false,'thresholds',rippleMasterDetector_threshold,'eventSpikeThreshold',eventSpikeThreshold); 
psthRipples = spikesPsth([],'eventType','ripples','numRep',500,'force',true,'minNumberOfPulses',10);
getSpikesRank('events','ripples');
...
UDStates = detectUpsDowns('plotOpt', true,'forceDetect',true,'NREMInts','all','useparfor',true,'excludeIntervals',[]);
psthUD = spikesPsth([],'eventType','slowOscillations','numRep',500,'force',true,'minNumberOfPulses',10,'restrict_to',[0 Inf]);
getSpikesRank('events','upstates');
