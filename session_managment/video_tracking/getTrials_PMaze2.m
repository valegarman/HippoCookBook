function [maze] = getTrials_PMaze2(varargin)
...
maze.polar_theta_limits = [15,10];

maze.boundary{1} = [0,25];
maze.boundary{2} = [0,-28];
maze.boundary{3} = [-5,20];
maze.boundary{4} = [3,20];
maze.boundary{5} = [maze.radius_in - 1, 7];
maze.boundary{6} = [maze.radius_in - 1, 2];

% Determining polar coordinates
[polar_theta,polar_rho] = cart2pol(tracking.position.y,tracking.position.x);
...
pos5test = find(diff(polar_theta < -boundary{5}(2))== -1 & tracking.position.x(1:end-1) < -2 & polar_rho(1:end-1) > boundary{5}(1)); % End of left rim
pos6test = find(diff(polar_theta > boundary{6}(2))== -1 & tracking.position.x(1:end-1) > 2 & polar_rho(1:end-1) > boundary{5}(1)); % End of right rim
...
% plot(tracking.position.x,tracking.position.y,'color',[.5 .5 .5]);
% hold on;
% scatter(tracking.position.x(pos1test),tracking.position.y(pos1test),'k');
% scatter(tracking.position.x(pos2test),tracking.position.y(pos2test),'k');
% scatter(tracking.position.x(pos3test),tracking.position.y(pos3test),'k');
% scatter(tracking.position.x(pos4test),tracking.position.y(pos4test),'k');
% scatter(tracking.position.x(pos5test),tracking.position.y(pos5test),'k');
% scatter(tracking.position.x(pos6test),tracking.position.y(pos6test),'k');
