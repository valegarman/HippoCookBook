function [behavior] = getSessionLinearize_pablo(varargin)
...
for ii = 1:size(sess,1)
    if sess(ii).isdir && ~isempty(dir([basepath filesep sess(ii).name filesep '*Tracking.Behavior.mat']))
        ...
        if strcmpi(tracking.apparatus.name, 'Linear Track  N-S') || isempty(tracking.apparatus.name)
            try
    ...
    %                 behaviorTemp.(sess(ii).name)= linearizeLinearMaze('verbose',verbose);
            ...
            fprintf('Not linearizing %s ',tracking.folder);
        end
    end
end
cd(basepath);
...
for ii = 1:size(efields,1)
    subfolder = find(ismember(tracking.folders,efields{ii}));
    preRec = tracking.events.subSessions(subfolder,1);
