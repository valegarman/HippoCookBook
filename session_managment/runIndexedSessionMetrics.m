...
dataForTable = [projectResults.cell_metrics.putativeCellType'...
    projectResults.geneticLine' ...
    num2cell(-projectResults.acgPeak.acg_time(1,projectResults.acgPeak.acgPeak_sample)')...
    num2cell(projectResults.cell_metrics.burstIndex_Royer2012')...
    num2cell(projectResults.cell_metrics.firingRate')...
    num2cell(projectResults.cell_metrics.troughToPeak' + 0.0083*rand(1,length(projectResults.cell_metrics.troughToPeak))') ...
...
VariableNames = ["putativeCellType",...
    "geneticLine", "acgPeak", "burstIndex","firingRate", "troughtToPeak", "brainRegion", "lightResponse", "rateDuringRipples",...
    "preferedThetaphase", "thetaModulation", "sessionName"];

unitsTable = cell2table(dataForTable,"VariableNames",VariableNames);
