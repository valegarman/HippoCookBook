...
for ii = 1:size(all_folders,1)
    if all_folders(ii).isdir && ~strcmpi(all_folders(ii).name,'.') && ~strcmpi(all_folders(ii).name,'..')
        ...
        if isempty(kilosortFolder)
            ...
            preprocessSession('basepath',pwd,'analogCh',analogCh,'spikeSort',spikeSort,'getPos',getPos, 'cleanArtifacts',cleanArtifacts,...
                'medianSubstr',medianSubstr,'tracking_pixel_cm',tracking_pixel_cm,'sessionSummary',sessionSummary);
        else 
