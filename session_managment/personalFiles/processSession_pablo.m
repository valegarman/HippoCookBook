...
if ~any(ismember(excludeAnalysis, {'12',lower('spatialModulation')}))
    try
        ...
        spatialModulation = computeSpatialModulation('force',true,'gridAnalysis',gridAnalysis,'randomization',randomization);
        if twoHalvesAnalysis
            firingMaps2Halves = firingMap2Halves(behavior,spikes,'pixelsPerCm',pixelsPerCm,'saveMat',true);
            placeFieldStats2Halves = computeFindPlaceFields2Halves('firingMaps',firingMaps2Halves,'useColorBar',false);
            spatialModulation2Halves = computeSpatialModulation2Halves('force',true,'tint',tint); % Not running gridAnalysis for two Halves
        end
    catch
