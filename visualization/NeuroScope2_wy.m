...
%
% By Peter Petersen
% Winnie Yang added ploting event start and end with different color, May
% 2022
...
%                 addLegend(labels_cell_types{unique_electrodeGroups(i)},num2strCommaSeparated(spike_identity_colormap(unique_electrodeGroups(i),:)));
    ...
    function plotEventData(t1,t2,colorIn1,colorIn2)
        ...
        if UI.settings.showEventsIntervals
            ...
            disp(size(statesData,1))

            c = [1 0 0; %red
                1 0 0;% green
                1 0 0; %red
                0 1 0];
            c = repmat(c,size(statesData,1),1);
            %disp([ydata2(1);ydata2(1);ydata2(2);ydata2(2)]*ones(1,size(statesData,1)));
            p1 = patch(double([statesData,flip(statesData,2)])',[ydata2(1);ydata2(1);ydata2(2);ydata2(2)]*ones(1,size(statesData,1)),'g','EdgeColor','flat','FaceVertexCData',c,'HitTest','off');
            alpha(p1,0.1);
        end
