...
allLFP = getLFP_temp(usechannels,'basepath',basePath,'basename',recordingname,...
...
normTHspec = bz_NormToRange(THmeanspec,[0 numusedchannels.*0.6]);
    ...
    specdt = mode(diff(t_FFT));
    thFFTspec = (abs(thFFTspec)); 
    thFFTspec(thFFTspec==0) = NaN;
    % [zFFTspec,mu,sig] = zscore(log10(thFFTspec)');
    mu = nanmean(log10(thFFTspec)');
    sig = nanstd(log10(thFFTspec)');
    zFFTspec = zeros(size(thFFTspec));
    for ii = 1:size(thFFTspec,2)
        zFFTspec(:,ii) = (log10(thFFTspec(:,ii)') - mu)./sig;
    end
