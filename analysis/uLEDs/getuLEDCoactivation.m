...
temp = zeros(size(boostrap_mat,1));
for ii = 1:size(boostrap_mat,1)
    for jj = 1:size(boostrap_mat,1)
        if any(boostrap_mat(ii,:) + boostrap_mat(jj,:) == 2) % if both neurons are coactivated at least by one light
            temp(ii,jj) = 1;
        end
    end
end
