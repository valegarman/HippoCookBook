function [optogeneticResponses] = getOptogeneticResponse_temp(varargin)
...
% Includes onset and offset (0 by default) to compute statistics. Also
% included in output variable
...
addParameter(p,'onset',0,@isnumeric); % in ms. Default: 0
addParameter(p,'offset',0,@isnumeric);% in ms. Default: 0
...
onset = p.Results.onset/1000;
offset = p.Results.offset/1000;
...
%% Adding onset and offet (0 otherwise)
if ~isempty(pulsesAnalog.timestamps)
    pulsesAnalog.timestamps(:,1) = pulsesAnalog.timestamps(:,1) + onset;
    pulsesAnalog.timestamps(:,2) = pulsesAnalog.timestamps(:,2) + offset;
end
if ~isempty(pulsesDigital.timestamps)
    pulsesDigital.timestamps(:,1) = pulsesDigital.timestamps(:,1) + onset;
    pulsesDigital.timestamps(:,2) = pulsesDigital.timestamps(:,2) + offset;
end
pulses.timestamps = [pulsesAnalog.timestamps; pulsesDigital.timestamps];  % combine pulses
%%
spikes = loadSpikes;
...
optogeneticResponses.onset = onset;
optogeneticResponses.offset = offset;
