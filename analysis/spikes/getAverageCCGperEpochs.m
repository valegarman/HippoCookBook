...
function [averageCCG] = getAverageCCGperEpochs(varargin)
...
% useDistinctShanks
%               Default, true
% epochs        By default, load epochs from session file.
%
% OUTPUTS
% averageCCG
%
...
targetFile = dir('*.getAverageCCGperEpochs.cellinfo.mat');
if ~isempty(targetFile) && ~force
    disp('Average CCG per epoch already computed! Loading file...');
...
for ii = 1:length(epochs)
    epochs_avgCCG{ii} = getAverageCCG('spikes',spikes,'basepath',basepath,'binSize',binSize,'winSize',winSize,'winSizePlot',winSizePlot,'plotOpt',false,'saveMat',false,...
        'force', true, 'skipStimulationPeriods', skipStimulationPeriods, 'winIndex', winIndex, 'interp0', interp0, 'useBrainRegions', useBrainRegions, 'useDistinctShanks', useDistinctShanks,...
        'includeIntervals',[epochs{ii}.startTime epochs{ii}.stopTime]);
end

avgCCGperEpoch.absCcgIndexShanks = [];
for ii = 1:length(epochs)
    avgCCGperEpoch.absCcgIndexShanks = cat(3,avgCCGperEpoch.absCcgIndexShanks, epochs_avgCCG{ii}.shanksCCG.absCcgIndexShanks);
end

if saveMat
    disp('Saving results...');
    filename = split(pwd,filesep); filename = filename{end};
    save([filename '.avgCCGperEpoch.cellinfo.mat'],'avgCCGperEpoch');
end
