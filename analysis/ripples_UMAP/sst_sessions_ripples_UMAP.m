...
% sessions = {'Z:\fSst1\fSst1_190605_sess35'};
sessions = {'Z:\fSst1\fSst1_190604_sess34','Z:\fSst1\fSst1_190605_sess35','Z:\fSst1\fSst1_190606_sess36'};
...
cell_sessions = {'fSst1_190604_sess34','fSst1_190605_sess35','fSst1_190606_sess36'};
% cell_sessions = {'fSst1_190605_sess35'};
...
for ii = 1:length(cell_sessions)
    cell_aux = find(taggedCells.hippo.sst(ismember(projectResults.session,lower(cell_sessions{ii}))));
...
for ii = 1:length(sessions)
    ...
    cell_mask = [cell_mask ones(1,length(find(taggedCells.hippo.sst(ismember(projectResults.session,lower(sess))))))*ii];
end
...
if saveFig
    ...
    saveas(s,'sst_amp_ripples_UMAP.fig');
end
...
if saveFig
    cd('C:\Users\pabad\OneDrive - imim.es\NeuralComputationLab\ActiveProjects\Bibliocampus\figures\Figures ripples UMAP');
    saveas(s,'sst_freq_ripples_UMAP.fig');
end
...
if saveFig
    cd('C:\Users\pabad\OneDrive - imim.es\NeuralComputationLab\ActiveProjects\Bibliocampus\figures\Figures ripples UMAP');
    saveas(s,'sst_spikes_ripples_UMAP.fig');
end
...
for ii = 1:size(y_mean_zscored,1)
    disp(['Computing sst: ', num2str(ii)]);
    sst_SI = SI.compute_structure_index(np.array(manifold(mask==ii,:)), np.array(y_mean_zscored(ii,1:1:length(find(mask==ii)))), int8(3), params = params );
    SI_sst_original(ii) = double(sst_SI{1});
    SI_sst_shuffled_original = double(sst_SI{4});
    SI_sst_99prctile_original(ii) = prctile(SI_sst_shuffled_original,99);
    SI_sst_95prctile_original(ii) = prctile(SI_sst_shuffled_original,95);
end
...
for ii = 1:size(y_mean_zscored,1)
    disp(['Computing sst: ', num2str(ii)]);
    sst_SI = SI.compute_structure_index(np.array(reduction(mask==ii,:)), np.array(y_mean_zscored(ii,1:1:length(find(mask==ii)))), int8(3), params = params );
    SI_sst_reduction(ii) = double(sst_SI{1});
    SI_sst_shuffled_reduction = double(sst_SI{4});
    SI_sst_99prctile_reduction(ii) = prctile(SI_sst_shuffled_reduction,99);
    SI_sst_95prctile_reduction(ii) = prctile(SI_sst_shuffled_reduction,95);
end
...
SST.SI_original.amp.value = SI_amp_original;
SST.SI_original.amp.prctile99 = SI_amp_99prctile_original;
SST.SI_original.amp.prctile95 = SI_amp_95prctile_original;

SST.SI_original.freq.value = SI_freq_original;
SST.SI_original.freq.prctile99 = SI_freq_99prctile_original;
SST.SI_original.freq.prctile95 = SI_freq_95prctile_original;

SST.SI_original.spikes.value = SI_sst_original;
SST.SI_original.spikes.prctile99 = SI_sst_99prctile_original;
SST.SI_original.spikes.prctile95 = SI_sst_95prctile_original;

SST.SI_reduction.amp.value = SI_amp_reduction;
SST.SI_reduction.amp.prctile99 = SI_amp_99prctile_reduction;
SST.SI_reduction.amp.prctile95 = SI_amp_95prctile_reduction;

SST.SI_reduction.freq.value = SI_freq_reduction;
SST.SI_reduction.freq.prctile99 = SI_freq_99prctile_reduction;
SST.SI_reduction.freq.prctile95 = SI_freq_95prctile_reduction;

SST.SI_reduction.spikes.value = SI_sst_reduction;
SST.SI_reduction.spikes.prctile99 = SI_sst_99prctile_reduction;
SST.SI_reduction.spikes.prctile95 = SI_sst_95prctile_reduction;

cd('C:\Users\pabad\OneDrive - imim.es\NeuralComputationLab\ActiveProjects\Bibliocampus\figures\Figures ripples UMAP');
save(['SST_structure_information_3sessions.mat'],'SST');
close all;
