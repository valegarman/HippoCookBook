...
% sessions = {'Z:\fNkx9\fNkx9_200824_sess6'};
sessions = {'Z:\fNkx8\fNkx8_200821_sess5','Z:\fNkx8\fNkx8_200901_sess12','Z:\fNkx8\fNkx8_200828_sess10','Z:\fNkx9\fNkx9_200824_sess6'};
...
cell_sessions = {'fNkx8_200821_sess5','fNkx8_200901_sess12','fNkx8_200828_sess10','fNkx9_200824_sess6'};
% cell_sessions = {'fNkx9_200824_sess6'};
...
for ii = 1:length(cell_sessions)
    cell_aux = find(taggedCells.hippo.id2(ismember(projectResults.session,lower(cell_sessions{ii}))));
...
for ii = 1:length(sessions)
    ...
    cell_mask = [cell_mask ones(1,length(find(taggedCells.hippo.id2(ismember(projectResults.session,lower(sess))))))*ii];
end
...
if saveFig
    ...
    saveas(s,'id2_amp_ripples_UMAP.fig');
end
...
if saveFig
    cd('C:\Users\pabad\OneDrive - imim.es\NeuralComputationLab\ActiveProjects\Bibliocampus\figures\Figures ripples UMAP');
    saveas(s,'id2_freq_ripples_UMAP.fig');
end
%% SPIKES
clear y
...
if saveFig
    cd('C:\Users\pabad\OneDrive - imim.es\NeuralComputationLab\ActiveProjects\Bibliocampus\figures\Figures ripples UMAP');
    saveas(s,'id2_spikes_ripples_UMAP.fig');
end
...
for ii = 1:size(y_mean_zscored,1)
    disp(['Computing id2: ', num2str(ii)]);
    id2_SI = SI.compute_structure_index(np.array(manifold(mask==ii,:)), np.array(y_mean_zscored(ii,1:1:length(find(mask==ii)))), int8(3), params = params );
    SI_id2_original(ii) = double(id2_SI{1});
    SI_id2_shuffled_original = double(id2_SI{4});
    SI_id2_99prctile_original(ii) = prctile(SI_id2_shuffled_original,99);
    SI_id2_95prctile_original(ii) = prctile(SI_id2_shuffled_original,95);
end
...
for ii = 1:size(y_mean_zscored,1)
    disp(['Computing id2: ', num2str(ii)]);
    id2_SI = SI.compute_structure_index(np.array(reduction(mask==ii,:)), np.array(y_mean_zscored(ii,1:1:length(find(mask==ii)))), int8(3), params = params );
    SI_id2_reduction(ii) = double(id2_SI{1});
    SI_id2_shuffled_reduction = double(id2_SI{4});
    SI_id2_99prctile_reduction(ii) = prctile(SI_id2_shuffled_reduction,99);
    SI_id2_95prctile_reduction(ii) = prctile(SI_id2_shuffled_reduction,95);
end
...
ID2.SI_original.amp.value = SI_amp_original;
ID2.SI_original.amp.prctile99 = SI_amp_99prctile_original;
ID2.SI_original.amp.prctile95 = SI_amp_95prctile_original;

ID2.SI_original.freq.value = SI_freq_original;
ID2.SI_original.freq.prctile99 = SI_freq_99prctile_original;
ID2.SI_original.freq.prctile95 = SI_freq_95prctile_original;

ID2.SI_original.spikes.value = SI_id2_original;
ID2.SI_original.spikes.prctile99 = SI_id2_99prctile_original;
ID2.SI_original.spikes.prctile95 = SI_id2_95prctile_original;

ID2.SI_reduction.amp.value = SI_amp_reduction;
ID2.SI_reduction.amp.prctile99 = SI_amp_99prctile_reduction;
ID2.SI_reduction.amp.prctile95 = SI_amp_95prctile_reduction;

ID2.SI_reduction.freq.value = SI_freq_reduction;
ID2.SI_reduction.freq.prctile99 = SI_freq_99prctile_reduction;
ID2.SI_reduction.freq.prctile95 = SI_freq_95prctile_reduction;

ID2.SI_reduction.spikes.value = SI_id2_reduction;
ID2.SI_reduction.spikes.prctile99 = SI_id2_99prctile_reduction;
ID2.SI_reduction.spikes.prctile95 = SI_id2_95prctile_reduction;

cd('C:\Users\pabad\OneDrive - imim.es\NeuralComputationLab\ActiveProjects\Bibliocampus\figures\Figures ripples UMAP');
save(['ID2_structure_information_3sessions.mat'],'ID2');
close all;
