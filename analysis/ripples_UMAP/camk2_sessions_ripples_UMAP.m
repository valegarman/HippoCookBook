...
sessions = {'Z:\fCamk1\fCamk1_200827_sess9','Z:\fCamk1\fCamk1_200901_sess12','Z:\fCamk3\fCamk3_201028_sess10_cleanned'};
% sessions = {'Z:\fCamk1\fCamk1_200827_sess9'};

clear unit

% cell_sessions = {'fCamk1_200827_sess9'};
cell_sessions = {'fCamk1_200827_sess9','fCamk1_200901_sess12','fCamk3_201028_sess10_cleanned'};
...
for ii = 1:length(cell_sessions)
    cell_aux = find(taggedCells.hippo.camk2(ismember(projectResults.session,lower(cell_sessions{ii}))));
...
for ii = 1:length(sessions)
    ...
    cell_mask = [cell_mask ones(1,length(find(taggedCells.hippo.camk2(ismember(projectResults.session,lower(sess))))))*ii];
end
...
if saveFig
    ...
    saveas(s,'camk2_amp_ripples_UMAP.fig');
end
...
if saveFig
    cd('C:\Users\pabad\OneDrive - imim.es\NeuralComputationLab\ActiveProjects\Bibliocampus\figures\Figures ripples UMAP');
    saveas(s,'camk2_freq_ripples_UMAP.fig');
end
%% SPIKES
clear y
...
if saveFig
    cd('C:\Users\pabad\OneDrive - imim.es\NeuralComputationLab\ActiveProjects\Bibliocampus\figures\Figures ripples UMAP');
    saveas(s,'camk2_spikes_ripples_UMAP.fig');
end
...
for ii = 1:size(y_mean_zscored,1)
    disp(['Computing camk2: ', num2str(ii)]);
    camk2_SI = SI.compute_structure_index(np.array(manifold(mask==ii,:)), np.array(y_mean_zscored(ii,1:1:length(find(mask==ii)))), int8(3), params = params );
    SI_camk2_original(ii) = double(camk2_SI{1});
    SI_camk2_shuffled_original = double(camk2_SI{4});
    SI_camk2_99prctile_original(ii) = prctile(SI_camk2_shuffled_original,99);
    SI_camk2_95prctile_original(ii) = prctile(SI_camk2_shuffled_original,95);
end
...
for ii = 1:size(y_mean_zscored,1)
    disp(['Computing camk2: ', num2str(ii)]);
    camk2_SI = SI.compute_structure_index(np.array(reduction(mask==ii,:)), np.array(y_mean_zscored(ii,1:1:length(find(mask==ii)))), int8(3), params = params );
    SI_camk2_reduction(ii) = double(camk2_SI{1});
    SI_camk2_shuffled_reduction = double(camk2_SI{4});
    SI_camk2_99prctile_reduction(ii) = prctile(SI_camk2_shuffled_reduction,99);
    SI_camk2_95prctile_reduction(ii) = prctile(SI_camk2_shuffled_reduction,95);
end
...
CAMK2.SI_original.amp.value = SI_amp_original;
CAMK2.SI_original.amp.prctile99 = SI_amp_99prctile_original;
CAMK2.SI_original.amp.prctile95 = SI_amp_95prctile_original;

CAMK2.SI_original.freq.value = SI_freq_original;
CAMK2.SI_original.freq.prctile99 = SI_freq_99prctile_original;
CAMK2.SI_original.freq.prctile95 = SI_freq_95prctile_original;

CAMK2.SI_original.spikes.value = SI_camk2_original;
CAMK2.SI_original.spikes.prctile99 = SI_camk2_99prctile_original;
CAMK2.SI_original.spikes.prctile95 = SI_camk2_95prctile_original;

CAMK2.SI_reduction.amp.value = SI_amp_reduction;
CAMK2.SI_reduction.amp.prctile99 = SI_amp_99prctile_reduction;
CAMK2.SI_reduction.amp.prctile95 = SI_amp_95prctile_reduction;

CAMK2.SI_reduction.freq.value = SI_freq_reduction;
CAMK2.SI_reduction.freq.prctile99 = SI_freq_99prctile_reduction;
CAMK2.SI_reduction.freq.prctile95 = SI_freq_95prctile_reduction;

CAMK2.SI_reduction.spikes.value = SI_camk2_reduction;
CAMK2.SI_reduction.spikes.prctile99 = SI_camk2_99prctile_reduction;
CAMK2.SI_reduction.spikes.prctile95 = SI_camk2_95prctile_reduction;

cd('C:\Users\pabad\OneDrive - imim.es\NeuralComputationLab\ActiveProjects\Bibliocampus\figures\Figures ripples UMAP');
save(['CAMK2_structure_information_3sessions.mat'],'CAMK2');
close all;