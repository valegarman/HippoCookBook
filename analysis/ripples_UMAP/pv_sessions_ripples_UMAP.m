...
% sessions = {'Z:\fPv4\fPv4_210311_sess4'};
sessions = {'Z:\fPv4\fPv4_210308_sess1','Z:\fPv4\fPv4_210311_sess4','Z:\fPv4\fPv4_210310_sess3'};
...
cell_sessions = {'fPv4_210308_sess1','fPv4_210311_sess4','fPv4_210310_sess3'};
% cell_sessions = {'fPv4_210311_sess4'};
...
for ii = 1:length(cell_sessions)
    cell_aux = find(taggedCells.hippo.pv(ismember(projectResults.session,lower(cell_sessions{ii}))));
...
for ii = 1:length(sessions)
    ...
    cell_mask = [cell_mask ones(1,length(find(taggedCells.hippo.pv(ismember(projectResults.session,lower(sess))))))*ii];
end
...
saveas(s,'pv_amp_ripples_UMAP.fig');
...
cd('C:\Users\pabad\OneDrive - imim.es\NeuralComputationLab\ActiveProjects\Bibliocampus\figures\Figures ripples UMAP');
saveas(s,'pv_freq_ripples_UMAP.fig');
...
cd('C:\Users\pabad\OneDrive - imim.es\NeuralComputationLab\ActiveProjects\Bibliocampus\figures\Figures ripples UMAP');
saveas(s,'pv_spikes_ripples_UMAP.fig');
...
for ii = 2:size(y_mean_zscored,1)
    disp(['Computing pv: ', num2str(ii)]);
    pv_SI = SI.compute_structure_index(np.array(manifold(mask==ii,:)), np.array(y_mean_zscored(ii,1:1:length(find(mask==ii)))), int8(3), params = params );
    SI_pv_original(ii) = double(pv_SI{1});
    SI_pv_shuffled_original = double(pv_SI{4});
    SI_pv_99prctile_original(ii) = prctile(SI_pv_shuffled_original,99);
    SI_pv_95prctile_original(ii) = prctile(SI_pv_shuffled_original,95);
end
...
for ii = 1:size(y_mean_zscored,1)
    disp(['Computing pv: ', num2str(ii)]);
    pv_SI = SI.compute_structure_index(np.array(reduction(mask==ii,:)), np.array(y_mean_zscored(ii,1:1:length(find(mask==ii)))), int8(3), params = params );
    SI_pv_reduction(ii) = double(pv_SI{1});
    SI_pv_shuffled_reduction = double(pv_SI{4});
    SI_pv_99prctile_reduction(ii) = prctile(SI_pv_shuffled_reduction,99);
    SI_pv_95prctile_reduction(ii) = prctile(SI_pv_shuffled_reduction,95);
end
...
PV.SI_original.amp.value = SI_amp_original;
PV.SI_original.amp.prctile99 = SI_amp_99prctile_original;
PV.SI_original.amp.prctile95 = SI_amp_95prctile_original;

PV.SI_original.freq.value = SI_freq_original;
PV.SI_original.freq.prctile99 = SI_freq_99prctile_original;
PV.SI_original.freq.prctile95 = SI_freq_95prctile_original;

PV.SI_original.spikes.value = SI_pv_original;
PV.SI_original.spikes.prctile99 = SI_pv_99prctile_original;
PV.SI_original.spikes.prctile95 = SI_pv_95prctile_original;

PV.SI_reduction.amp.value = SI_amp_reduction;
PV.SI_reduction.amp.prctile99 = SI_amp_99prctile_reduction;
PV.SI_reduction.amp.prctile95 = SI_amp_95prctile_reduction;

PV.SI_reduction.freq.value = SI_freq_reduction;
PV.SI_reduction.freq.prctile99 = SI_freq_99prctile_reduction;
PV.SI_reduction.freq.prctile95 = SI_freq_95prctile_reduction;

PV.SI_reduction.spikes.value = SI_pv_reduction;
PV.SI_reduction.spikes.prctile99 = SI_pv_99prctile_reduction;
PV.SI_reduction.spikes.prctile95 = SI_pv_95prctile_reduction;

cd('C:\Users\pabad\OneDrive - imim.es\NeuralComputationLab\ActiveProjects\Bibliocampus\figures\Figures ripples UMAP');
save(['PV_structure_information_3sessions.mat'],'PV');
