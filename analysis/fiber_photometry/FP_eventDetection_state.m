function [Event_Features]=FP_eventDetection_state(fiber, rec_type,varargin)
...
green=fiber.green_PP.channel_dFF_Smoothed;
% green=fiber.green_fpa.fNormalized;
% green=fiber.green.data;
samplingRate=fiber.sr;
t10=round(10*samplingRate); %nº timestamps for 10 seconds
t8=round(8*samplingRate); %nº timestamps for 10 seconds
...
if ~contains(rec_type,'Maze') %Default parameters for Home cage recordings:
    % Varargin
    p = inputParser();
...
else %Default parameters for Maze recordings:

    % Varargin
    p = inputParser();
    
    addParameter(p,'Smooth_factor',[t05 t05]); 
    addParameter(p,'win_sz',[t15 0]);
    addParameter(p,'gap',t02,@isnumeric); 
    addParameter(p,'ED_range_up',2.5,@isnumeric); 
    addParameter(p,'event_sep',t02,@isnumeric); 
    addParameter(p,'min_length',t05,@isnumeric); 
    addParameter(p,'MinProminence_start',0.0015,@isnumeric);
    addParameter(p,'MinProminence_end',0.0015,@isnumeric);
    addParameter(p,'time_binning',false);
    addParameter(p,'n_bins',10,@isnumeric);
    addParameter(p,'plt',false)
...
signal=smoothdata(green,'movmean',Smooth_factor); %smoothdata first so that it is easier to compute the events
