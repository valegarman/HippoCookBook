...
if length(isosbestic.data) ~= length(red.data)
    data_length = min(length(isosbestic.data),length(red.data));
    isosbestic.data=isosbestic.data(1:data_length);
    isosbestic.timestamps=isosbestic.timestamps(1:data_length);
    red.data=red.data(1:data_length);
    red.timestamps=red.timestamps(1:data_length);
end 
...
plot(fiber.timestamps, fiber.green_PP.iso)
title('Iso preprocessed') 

fig4=figure('Name','PP fiber green');
plot(fiber.timestamps, fiber.green_PP.channel_dFF_Smoothed)
