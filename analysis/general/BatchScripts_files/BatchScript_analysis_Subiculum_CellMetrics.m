...
sessionsTable = readtable([HCB_directory.path filesep 'indexedSessions_SubiculumProject_OF.csv']); % the variable is called allSessions
forceReload = false;
...
for ii = 1:length(sessionsTable.SessionName)
    if strcmpi(sessionsTable.Project{ii}, targetProject) || strcmpi('all', targetProject)
        ...
        cell_metrics = ProcessCellMetrics('session', session,'manualAdjustMonoSyn',false,'forceReload',true);
...
%         end
        ...
        % ===========================
%         if ~isempty(dir('*spatialModulation.cellinfo.mat'))
%             file = dir('*spatialModulation.cellinfo.mat');
%             load(file.name);
%             if spatialModulation.speedThresh ~= 1
%                 speedThresh = 1; % 5cm/s
%                 pixelsPerCm = 2.5;
%                 randomization = true;
%                 gridAnalysis = false;
