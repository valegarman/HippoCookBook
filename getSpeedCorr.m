...
addParameter(p,'threshold',2,@isscalar);
...
if ~isempty(dir([session.general.name,'.Speed.events.mat']))
    disp('Speed.events.mat found. Loading file');
    file = dir([session.general.name,'.Speed.events.mat']);
    load(file.name);
end
...
for j = 1:length(spikes.UID)
    try
        ...
        valid_idx2(speed.velocity < threshold) = 0;
        fr = a(:,2);
        fr(~valid_idx1) = NaN;
        vel = speed.velocity;
        vel(~valid_idx2) = NaN;
...
speed_val = zeros(numQuantiles,length(spikes.times),2);
speed_fr_corr = zeros(length(spikes.times),2);
ls = zeros(length(spikes.times),3,2);
...
if mkplt
    ...
    figure, hold on
    colors = {'m','r', 'c','g','b','k'};
    for shanki = 1:size(session.extracellular.electrodeGroups.channels)
        try
